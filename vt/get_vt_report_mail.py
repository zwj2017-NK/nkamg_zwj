...
def send_email(msg_string):
  ...
  server.sendmail(from_addr, [to_addr, 'zwang@nankai.edu.cn', '1253848320@qq.com' ], msg.as_string())#
  server.quit()
...
def get_report():
  with open('/home/nkamg/virusshare_todo_2/todo.txt','rb') as f1:
    temp = f1.read().split('\n')
    temp.remove('')
  time_string = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
  if os.path.exists('/home/nkamg/virusshare_todo_2/result_' + time_string[:10] + '.json'):
    with open('/home/nkamg/virusshare_todo_2/result_' + time_string[:10] + '.json', 'rb') as f2:
...
# Parallelization
#-------------------------------
def parallel(worker, jobs, n_process_per_cpu=2, res_save=None):
  """
  Parameters:
  1. worker: a function to finish the given job
  2. jobs: a list of jobs
  3. n_process_per_cpu: the number of process for each CPU
  4. res_save: a function to dynamically save list_res

  Return:
  A list of worker list_res
  """
  N_jobs = len(jobs)
  N_workers = mp.cpu_count() * n_process_per_cpu
  p_list = []
  ...
  for i in range(n_apikey):
    p = mp.Process(target=worker, args=(q_task, q_result, i,))
    p_list.append(p)
    p.start()

  widgets = [pb.FormatLabel('Processed: %(value)s/{} samples (in: %(elapsed)s)'.format(N_jobs)), pb.Percentage(), pb.Bar()]
  pbar = pb.ProgressBar(widgets=widgets, maxval=N_jobs).start()
  ll = 0
  while True:
    list_res = []
    q_len = q_result.qsize()
    ...
    if res_save:
      if len(list_res) == 0:
        ...
        res_save(list_res)
        update_todo([i[1] for i in list_res])
...
def update_todo(list_res):
  if len(list_res) == 0:
    return
  regex = re.compile('\s+')
  with open('/home/nkamg/virusshare_todo_2/todo.txt','rb') as f:
  ...
  for i in list_res:
    try:
      ...
      pass
  with open('/home/nkamg/virusshare_todo_2/todo.txt','wb') as e:
    for j in list_todo:
      e.write(j + '\n')
  return
...
def write_res(list_res):
  time_string = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
  if os.path.exists('/home/nkamg/virusshare_todo_2/result_' + time_string[:10] + '.json'):
    with open('/home/nkamg/virusshare_todo_2/result_' + time_string[:10] + '.json', 'rb') as e:
  ...
  if len(dict_total.keys()) is not 0:
    ...
    with open('/home/nkamg/virusshare_todo_2/result_' +time_string[:10] + '.json','w') as f:
      f.write(str_json)
  return
    
def do_task(q_read, q_res, seq):
  f_time = 0
...
def main():
  ...
  time_string = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
  if os.path.exists('/home/nkamg/virusshare_todo_2/result_' + time_string[:10] + '.json'): 
    with open('/home/nkamg/virusshare_todo_2/result_' + time_string[:10] + '.json', 'rb') as g:
  ...
  for i in list_done:
    try:
      ...
      pass
  with open('/home/nkamg/virusshare_todo_2/todo.txt','wb') as h:
    for j in list_todo:
      h.write(j + '\n')
  print 'the task starts'
  try:
    parallel(do_task, list_todo, res_save=write_res)
  except Exception,e:
    pass
